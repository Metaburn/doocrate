...
describe('## Doocrate Rules', () => {
  ...
  describe('## projects', async () => {
    it('should let anyone create a project', async () => {
      const db = authedApp({ uid: 'alice' });
      const project = db.collection('projects').doc('new_project');
      await firebase.assertSucceeds(
        project.set({
          isPublic: true,
  ...
  describe('### Tasks', async () => {
    const privateProjectId = 'private_project';
    ...
    const invitedTaskParticipant = {
      email: 'guy@test.com',
      uid: 'guy',
      db: null,
    };

    const uninvitedTaskParticipant = {
    ...
    beforeEach(async () => {
      ...
      // Create the project
      const project = adminDb.collection('projects').doc(privateProjectId);
      await firebase.assertSucceeds(
        project.set({
          isPublic: false,
      ...
      const invites = [aliceTaskCreator.email, invitedTaskParticipant.email];
      await initializeInvites(adminDb, privateProjectId, invites);
      ...
      // should create a task for an invited user
      await firebase.assertSucceeds(
        aliceTaskCreator.db
          .collection('projects')
          .doc(privateProjectId)
          .collection('tasks')
          .doc(taskId)
          .set({
    ...
    it('should allow self assign non-assigned task for invited', async () => {
      const invitedDb = authedApp({
        uid: invitedTaskParticipant.uid,
        email: invitedTaskParticipant.email,
      });

      const unInvitedDb = authedApp({
        uid: uninvitedTaskParticipant.uid,
        email: uninvitedTaskParticipant.email,
      });

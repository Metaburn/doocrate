...
import {
  getUrlSearchParams,
  removeQueryParam,
} from 'src/utils/browser-utils.js';
...
class TaskFilters extends Component {
  ...
  onCSVLink() {
    this.setState({
      CSVLink: (
        <span>
          <CSVLink className={'button-as-link'} data={this.props.generateCSV()}>
            Download Tasks as CSV (Admin)
          </CSVLink>
        </span>
      ),
    });
  ...
  FilterLink = ({ id }) => {
    const project =
      this.props.selectedProject &&
      this.props.selectedProject.url !== 'undefined'
        ? this.props.selectedProject.url
        : this.props.userDefaultProject;
    ...
    return (
      <NavLink
        ...
        isActive={isActive}
        to={{ pathname: defaultTask, search }}
      >
        {linkTitle}
      </NavLink>
    );
  ...
  render() {
    ...
    if (this.props.isAdmin) {
      downloadCSV = this.state.CSVLink ? (
        this.state.CSVLink
      ) : (
        <button
    ...
    return (
      <nav className="task-filters">
        ...
        <div className="task-type">
          <NavLink
            ...
            isActive={(match, location) =>
              TaskFilters.getFilterQuery(location) === 'mine'
            }
            to={{
          ...
          <NavLink
            className={'filter-link'}
            isActive={(match, location) =>
              TaskFilters.getFilterQuery(location) === 'unassigned'
            }
            to={{
              pathname: defaultTask,
          ...
          <NavLink
            className={'filter-link'}
            isActive={(match, location) =>
              TaskFilters.getFilterQuery(location) === 'critical'
            }
            to={{
              pathname: defaultTask,
          ...
          <NavLink
            className={'filter-link'}
            isActive={(match, location) =>
              TaskFilters.getFilterQuery(location) === undefined
            }
            to={{
              pathname: defaultTask,
        ...
        <button onClick={this.props.onApply} className={'apply-btn'}>
          {i18n.t('filter.apply')}
        </button>
        {downloadCSV}
      </nav>

...
export class AdminDashboard extends Component {
  constructor() {
    ...
    this.setAllUsersToHaveCreateTaskAssignPermissions = this.setAllUsersToHaveCreateTaskAssignPermissions.bind(
      this,
    );
  ...
  onCSVLink() {
    this.setState({
      CSVLink: (
        <span>
          <CSVLink className={'button-as-link'} data={this.generateUsersCSV()}>
            הורדת דוח משתמשים
          </CSVLink>
        </span>
      ),
    ...
    return (
      <div className="admin-dashboard">
        ...
        {this.state.CSVLink ? (
          ...
          >
            הכן הורדה של רשימת המשתמשים
          </button>
        )}
        <br />
        <Button onClick={this.unlockDashboard}>
          אפשר את כל הכפתורים הבאים - מנגנון אזהרה
        </Button>
        <hr />
        <br />
        <h1>1.פתיחת אפשרות למשתמשים ליצור ולקחת משימות</h1>
        <Button
          disabled={!this.state.isButtonsUnlocked}
          onClick={this.setAllUsersToHaveCreateTaskAssignPermissions}
        >
          הפוך את כל המשתמשים של המערכת לבעלי הרשאה ליצור משימות ולקחת משימות
        </Button>
        ...
        {this.renderUsersWhoDidntBuy(
          'usersWhoDidntBuy',
          'משתמשים שלא קנו כרטיס מחולקים בפסיק לדוגמא galbra@test.com,john@gmail.com',
          this.state.isButtonsUnlocked,
          1,
        )}
        ...
        <Button
          disabled={!this.state.isButtonsUnlocked}
          onClick={this.setUsersToDidntBuy}
        >
          הדלק את הדגל עבור את המשתמשים הללו ל-DidntBuy
        </Button>
        ...
        {this.renderInput(
          'usersToAllowToRegisterCount',
          'מספר משתמשים לפתוח עבורם הרשמה. נגיד 100',
          this.state.isButtonsUnlocked,
          2,
          false,
        )}
        ...
        <Button
          disabled={!this.state.isButtonsUnlocked}
          onClick={this.setUsersAllowCreateTasks}
        >
          פתח את האפשרות לפתוח משימה ולקחת משימה לאנשים הללו
        </Button>
        {this.state.successEmails && this.state.successEmails.length > 0 ? (
          <div>
            <span>אימיילים שנפתחה עבורם האפשרות להרשם:</span>
            <br />
  ...
  setUsersAllowCreateTasks() {
    ...
    this.props.showSuccess(
      'People who didnt have ticket and now have ' +
        dontHaveTicketCounter +
        ' People\r\n' +
        'People who have ticket ' +
        haveTicketCounter,
    );
  }
  ...
  setUsersToDidntBuy() {
    ...
    this.state.usersWhoDidntBuy.forEach(userWhoDidntBuyEmail => {
      ...
      if (!targetUserId) {
        ...
        this.props.showError(
          'Cant find a user by the email ' +
            userWhoDidntBuyEmail +
    ...
    this.props.showSuccess(
      'Updating ' +
        this.state.usersWhoDidntBuy.length +
        ' Users... - all have flag of didntBuy. Check console and wait for it to reach ' +
        this.state.usersWhoDidntBuy.length,
    );
  }
  ...
  setAllUsersToHaveCreateTaskAssignPermissions() {
    ...
    this.props.showSuccess(
      'Updating ' +
        ...
        ' Users... - all have permission to create tasks and assign tasks. Check console and wait for it to reach ' +
        this.state.users.size,
    );
  }

...
export class MePage extends Component {
        ...
        {(t, { i18n }) => (
          <div className="me notranslate">
            ...
            <MyTasks
              ...
              setTour={(isShow, step) => {
                this.setTourAndGo(isShow, step);
              }}
            />

            {auth && auth.role !== 'user' ? (
              <div className={'projects-you-manage'}>
                {t('my-space.projects-you-manage')}:
              </div>
            ) : (
              ''
            )}
  ...
  renderAdminProjects = t => {
    ...
    if (!auth || !auth.adminProjects || auth.adminProjects.size <= 0) return;
              ...
              <div className={'project'} key={project}>
                ...
                <span>
                  <NavLink to={`/${project}/reports`}>
                    {t('my-space.project-report')}
                  </NavLink>
                </span>
                <span>
                  <NavLink to={`/${project}/edit`}>
                    {t('my-space.edit-project')}
                  </NavLink>
                </span>
              </div>
            );
